...
class Command(BaseCommand):
    help = 'Run all deployment setup commands: migrate, collect static, ensure admin user'
    ...
    def handle(self, *args, **options):
        ...
        if created:
            # Set a secure default password (should be changed via Django admin)
            admin_user.set_password('ChangeMe123!')
            admin_user.save()
            self.stdout.write(self.style.SUCCESS(f'✓ Admin user created: admin / ChangeMe123!'))
        else:
